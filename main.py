...
# --- MÓDULO: QUADRO DE CARGAS  ---
    ...
    if st.session_state.get('qdc_pronto', False):
        ...
        # --- AQUI COMEÇA O SEU NOVO CÓDIGO DE PDF ---
        try:
            from fpdf import FPDF
            ...
            pdf.add_page()
            pdf.set_font("Arial", "B", 16)
            pdf.cell(190, 10, "VoltSpec Pro - Memorial de Calculo", ln=True, align="C")
            pdf.set_font("Arial", "", 10)
            pdf.cell(190, 7, "Responsavel: Danilo Reis | NBR 5410", ln=True, align="C")
            pdf.ln(10)

            # Tabela no PDF
            pdf.set_font("Arial", "B", 11)
            pdf.cell(190, 8, "Listagem de Circuitos:", ln=True)
            pdf.set_font("Arial", "", 8)
            
            for c in circuitos:
                # Limpa acentos para o PDF não dar erro
                txt = f"{c['Circuito']} | {c['Descrição']} | Pot: {c['Potência']} | Disj: {c['Disjuntor']} | Cabo: {c['Cabo']}"
                txt = txt.encode('ascii', 'ignore').decode('ascii')
                pdf.cell(190, 7, txt, ln=True, border=1)

            pdf_out = pdf.output(dest="S")
            pdf_bytes = pdf_out.encode("latin-1", errors="replace") if isinstance(pdf_out, str) else pdf_out

            st.download_button(
                label="📥 Baixar Memorial Técnico em PDF",
                data=pdf_bytes,
                file_name="VoltSpec_Memorial.pdf",
                mime="application/pdf",
                use_container_width=True
            )
        except Exception as e:
...
>>>>>>> fb89177 (Adicionando funcionalidade de PDF)
